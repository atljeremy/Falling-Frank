...
static const int FEET_TILL_GROUND = 10000;

@interface LevelTwoLayer()
// Clouds 
...
- (id)init
{
    if (self = [super init]) {
        ...
        CCLabelTTF* levelLabel = [CCLabelTTF labelWithString:@"Level 2" fontName:@"Marker Felt" fontSize:18 dimensions:labelSize hAlignment:NSTextAlignmentRight];
...
- (void)incrementTime
{
    ...
    if (self.time == 32 || self.time == 100) {
        [self showTempInvincibility];
    }
    
    if (self.time == 68 || self.time == 141) {
        [self showTempSlowMo];
    }
...
- (void)nextFrame:(ccTime)dt
{
    ...
    float birdOneSpeed   = (self.isSlowMoActive) ? 50  : 125;
    float birdTwoSpeed   = (self.isSlowMoActive) ? 75  : 175;
    float birdThreeSpeed = (self.isSlowMoActive) ? 100 : 225;
...
{
    CGSize size = [[CCDirector sharedDirector] winSize];
    __block LevelTwoLayer* _self = self;
...
//        [[SimpleAudioEngine sharedEngine] resumeBackgroundMusic];
//        [[CCDirector sharedDirector] resume];
//        [[CCDirector sharedDirector] replaceScene:[CCTransitionFade transitionWithDuration:1.0 scene:[LevelThreeLayer scene]]];
//    }];
